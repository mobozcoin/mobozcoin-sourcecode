...
// Copyright (c) 2011-2012 Mobozcoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("Mobozcoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MobozcoinTEST0\r");
            Send(hSocket, "WHO #MobozcoinTEST0\r");
        } else {
            // randomly join #Mobozcoin00-#Mobozcoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Mobozcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Mobozcoin%02d\r", channel_number).c_str());
        }
